window.debugData={};window.loadServerStatus=async function(){const t=document.getElementById("server-status");if(t){t.className="result loading",t.textContent="Loading server status...";try{const s=await(await fetch("/debug",{method:"GET"})).json();window.debugData=s,t.className="result success",t.innerHTML=`
                ...
                `}catch(e){t.className="result error",t.innerHTML=`<strong>Error:</strong> ${e?.message||"Unknown error"}`}}};window.testAPI=async function(t){const e=document.getElementById("api-test");if(e){e.className="result loading",e.textContent=`Testing ${t}...`;try{const n=await(await fetch("/debug",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"test-api",endpoint:t})})).json(),o=n.status!==404;e.className=o?"result success":"result error",e.innerHTML=`
                ...
                `}catch(s){e.className="result error",e.innerHTML=`<strong>Error testing ${t}:</strong> ${s?.message||"Unknown error"}`}}};window.testAllAPIs=async function(){const t=["/api/auth","/api/auth/register","/api/auth/verify-otp","/api/auth/me","/api/test-auth","/api/check-env"],e=document.getElementById("all-api-results");if(!e)return;e.innerHTML='<div class="result loading">Testing all endpoints...</div>';const s=[];for(const r of t)try{const i=await(await fetch("/debug",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"test-api",endpoint:r})})).json();s.push({endpoint:r,status:i.status,success:i.status!==404})}catch(a){s.push({endpoint:r,status:"ERROR",success:!1,error:a?.message||"Unknown error"})}const n=s.filter(r=>r.success).length,o=s.length;e.innerHTML=`
            ...
            `,window.updateDiagnosis(s)};window.updateDiagnosis=function(t){const e=document.getElementById("diagnosis");if(!e)return;const s=t.filter(r=>r.success).length,n=t.length;let o="";s===0?o=`
